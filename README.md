...
A microservices-based real-time chat application built with TypeScript and Node.js. This project implements an API Gateway pattern with event-driven architecture, featuring JWT authentication, multi-database persistence, and Redis caching.
...
```bash
git clone git@github.com:Tahsin005/chatapp-microservices.git
...
```bash
pnpm --filter @chatapp/gateway-service dev
pnpm --filter @chatapp/auth-service dev
pnpm --filter @chatapp/user-service dev
pnpm --filter @chatapp/chat-service dev
```
